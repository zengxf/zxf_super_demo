...
public class HtmlToImage4 {
	...
	URI contentUri = HtmlToImage4.class.getResource( "test.html" ).toURI();
	HtmlToImageUtil.log( "contentUri: %s", contentUri );
	String contentHtml = HtmlToImageUtil.toString( contentUri, "UTF-8" );

	URI templateUri = HtmlToImage4.class.getResource( "morning-template.html" ).toURI();
	String templateHtml = HtmlToImageUtil.toString( templateUri, "UTF-8" );

	String html = templateHtml.replace( "#content#", contentHtml );
	String headImagePath = HtmlToImageUtil.class.getResource( "morning-head_bg.png" ).toURI().toString();
	String paperImagePath = HtmlToImageUtil.class.getResource( "morning-paper_bg.png" ).toURI().toString();
	...
	if ( System.getProperty( "os.name" ).startsWith( "Windows" ) ) {
	    ...
	    saveFile = HtmlToImage4.class.getResource( "" ).getPath() + "test-linux.png";
	}
